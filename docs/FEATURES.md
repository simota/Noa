...
- **DA / DSR 応答** — DA1 `ESC[?62;22c`、カーソル位置レポート等
...
制限: Sixel は非対応(画像経路は Kitty グラフィックスのみ)。

## ウィンドウ・タブ・分割(noa-app)

- **マルチウィンドウ / ネイティブタブ** — 新規 / 閉じる / 番号選択 / 前後移動
- **分割ツリー(Splits)** — 右 / 下分割、方向フォーカス移動、リサイズ、均等化、ズームトグル
...
`~/Library/Application Support/noa/config` から読み込み。TOML と Ghostty 形式の両対応、CLI フラグが上書き。
...
| 挙動 | `scrollback-limit`, `clipboard-read`, `clipboard-paste-protection`, `alpha-blending`, `config-file`(インクルード) |
| macOS | `macos-option-as-alt`, `macos-titlebar-style` |
| Quick Terminal | `quick-terminal-hotkey/-size/-autohide` |
| サイドバー | `sidebar-enabled/-width/-hotkey` |

- **Ghostty 設定インポート** — 移行統計付きインポート
- `keybind =` キーは認識するが現状値は破棄(キー再割り当て未対応)
