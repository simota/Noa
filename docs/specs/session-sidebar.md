...
- **FR-11 Status dots**: busy (OSC 133 `has_running_program`) = blue / idle = green / unread bell = yellow status dot. The unread bell is drained by the io thread from `Terminal::take_pending_bell` (terminal.rs:305, sourced from BEL) and sent as a SessionDelta, cleared once that session's window is focused.
...
- **FR-16 Attention (awaiting-response indicator)**: when a pane in a non-focused window issues an OSC 9/777 desktop notification (typically Claude Code / Codex / agy notifying that it's waiting for interactive input), an `attention` flag is set on that session card. This shows as (a) status dot = red (priority: attention > bell > busy > idle), (b) `· awaiting response` appended to the process line in the attention color, (c) an `● ` prefix on the title band in the tab overview (unread bells are marked the same way). Cleared together with the unread bell when the window gains focus. Notifications on the currently focused window don't raise attention, for the same reason desktop notifications are suppressed there (the user is already looking at it).
