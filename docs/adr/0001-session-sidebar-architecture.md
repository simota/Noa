...
4. **可視性 = per-window トグル**: config `sidebar-enabled` がアプリ全体の初期値、hotkey はフォーカス中ウィンドウのみ反転。「モード切替（sidebar↔native）」はこのトグルに縮退（A-flavor ではネイティブタブが無傷 = 非表示が従来モード）。
