...
## Amendment v2 — 表示ホストの変更(2026-07-06, ユーザー指示)

専用 NSWindow(タブグループ外・最大化)をやめ、**トグル時にフォーカスしていたターミナルウィンドウ(ホスト)内のオーバーレイ**として表示する。実装は `OverviewWindowState` を `host: WindowId` ベースへ変更し、ホストのサーフェスへ present、ホストウィンドウの入力(redraw / キー / マウス / スクロール / IME)を Overview 表示中だけ Overview キーマップへルーティングする(構造イベント — resize / focus / occlusion / close — は通常のウィンドウ処理へフォールスルー)。

- **上書きされる決定**: Magi 決定4「専用ウィンドウはタブグループ外」、REQ-OV-1/OV-2 の「専用ウィンドウ」文言、SHAPE のウィンドウライフサイクル節。REQ-OV-2 の目的(タブ巡回/カウントに Overview が現れない)はホスト方式では構造的に自明(専用 WindowId が存在しない)。
- **付随する挙動変更**: タイル活性化(クリック / Return / Cmd+n)は Exposé 同様オーバーレイを**ディスミス**する(旧: 専用ウィンドウは開いたまま = 常設ダッシュボード)。出しっぱなし監視は、任意の1ウィンドウを Overview 表示のままにして他ウィンドウで作業することで引き続き可能。ホストのタブを閉じるとオーバーレイも破棄。ホスト自身も監視対象タイルに含まれる(自己除外は不要になった)。
- **不変**: グリッドレイアウト / 9枚上限+placeholder 退化 / 10Hz スロットル / 共有スクラッチ+blit / 検索・ヒントバー / Overview キーマップ / PTY 非パススルー(REQ-OV-7)。
...
- **パリティ例外(⚠E, 確定)**: Session Overview は Ghostty に対応機能が存在しない、本リポジトリ初の「忠実クローン哲学からの意図的な逸脱」であり、Ghostty パリティ照合の対象外として L0 に記録する。
...
- **REQ-OV-1** [MH]: 専用キーバインド+ネイティブメニュー項目で Session Overview 専用ウィンドウをトグル表示/非表示する(`ToggleSplitZoom` と同型のトグルコマンド、`KeybindEngine`/`AppCommand`/`macos_menu.rs` の確立パターンを踏襲)。
...
- **⚠E Ghostty パリティ例外文言(確定・L0 反映済み)**: 「Session Overview は Ghostty に対応機能が存在しない、本リポジトリ初の『忠実クローン哲学からの意図的な逸脱』であり、Ghostty パリティ照合の対象外として L0 に記録する。」*LOCK 承認済み (2026-07-03)。*
...
- 現状ギャップの棚卸し(コード裏付け、下記 REQ で個別に要件化):
  ...
  - タブ検索フィルタは存在しないが、`command_palette.rs:139` `command_palette_filter` / `command_palette.rs:151` `is_subsequence_ci` という非連続部分列(subsequence)マッチの手書き実装が既にある。ただしモックアップの「Search sessions」はタイトルの**部分一致(substring)**であり意味論が異なるため、パターンは参考にしつつ新規関数として要件化する(REQ-OV-16)。
