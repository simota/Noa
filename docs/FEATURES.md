...
- **Glassmorphism mode** — `glassmorphism` (`off`/`1`/`2`/`3`, default `off`, higher = more transparent): translucent frosted sidebar / tab-overview / titlebar-tab-bar chrome and frosted command palette / prompts / dialogs over the blurred desktop; any on-level takes over `background-opacity` / `background-blur-radius` with that level's own recommended pair
