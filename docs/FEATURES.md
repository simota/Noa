...
全キーの型、許容値、既定値、clamp / fallback 規則は [CONFIGURATION.md](CONFIGURATION.md)、
既定キーバインドと全 action 名は [KEYBINDINGS.md](KEYBINDINGS.md) を参照。
...
- **セキュアキーボードエントリ** — トグル可能
- **Titlebar Proxy Icon** — focus 中 pane の OSC 7 pwd を titlebar の proxy icon に反映。`macos-titlebar-proxy-icon = hidden` で無効化
- **Quick Look 単語検索** — trackpad force click(deep press)で pointer 下の単語を辞書ポップアップ表示。選択状態は変更しない
