...
| `scrollback-persist-max-age-days` | non-negative integer (days) | `7` | noa-specific key. Snapshots older than this are discarded at launch. `0` never expires. Days, as a plain integer — noa's config has no unit-suffix syntax, so `7d` is rejected |
