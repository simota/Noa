...
- **WebSocket over TCP**: `ws://<server-bind>:<server-port>/`(既定 bind `127.0.0.1`、既定ポート `61771`)。TLS なし。既定は loopback 限定 bind — `server-bind`(例 `0.0.0.0`)を設定すると LAN 上の他ホストから直接到達できる opt-in の bind アドレスを選べる(v2)。TLS が無いことに変わりはないため、LAN bind は信頼できるネットワークでのみ使うこと。信頼できないネットワークではリモートからは従来どおり SSH/Tailscale 等のトンネル終端で到達させる。
