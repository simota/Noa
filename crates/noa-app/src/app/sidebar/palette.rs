...
use crate::theme_settings::{
    ATTRIBUTE_CHIP_HINT, Liveness, RowDraft, SettingsRowKind, Swatch, ThemeSettings,
    ThemeSettingsMode, attribute_chip_segments, contrast_label, favorites_chip_label, footer_text,
    match_count_label, sample_lines, sample_swatches, settings_row_display_value,
};
...
fn settings_rows_overlay_text(
    ...
    cup(&mut out, hint_row, LIST_COL);
    // Settings-mode footer: this path owns R-5 search + R-7 reset, so it keeps
    // the search-aware settings hint (Tab = search under the reconciled
    // routing). The Theme picker path (`theme_picker_overlay_text`) uses the
    // shared mode-aware `footer_text` instead.
