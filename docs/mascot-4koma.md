...
*あるある: your tests pass, CI fails, and the "fix" is absurd.*
...
### #2 — Rubber Duck
*あるある: saying the bug out loud solves it — the "listener" did nothing.*
...
*あるある: three hours of debugging, and the fix was one character.*
...
### #6 — --force
*あるある: force-push nukes the team's work — but git never truly forgets.*
...
*あるある: "one more thing before bed" loops until sunrise.*
...
*あるある: adding a log hides the bug — so the "temporary" log ships forever.*
